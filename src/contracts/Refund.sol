...
contract Refund is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public  PUBLIC_PRICE = 0.01 ether; 
    address public PAYMNET_ADDRESS;
    string private _baseURIextended;

    constructor(address _payment, string memory _url) ERC721A("Refund", "RFD") {
        _baseURIextended = _url;
        PAYMNET_ADDRESS = _payment;
    }
    ...
    function mint(uint256 nMints) external payable nonReentrant {
        ...
        _withdraw(PAYMNET_ADDRESS, msg.value);
    }

    function airdrop(address airdropAddress, uint256 nMints) external onlyOwner {
        require(totalSupply() + nMints <= MAX_SUPPLY, "Mint exceeds total supply");
        _safeMint(airdropAddress, nMints);
    }
    ...
    function changePaymentAddress(address _paymentAddress) external onlyOwner {
        PAYMNET_ADDRESS = _paymentAddress;
    }
